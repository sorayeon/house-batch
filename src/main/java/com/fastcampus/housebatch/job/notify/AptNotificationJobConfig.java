...
public class AptNotificationJobConfig {
    ...
    public Step aptNotificationStep(
            ...
            ItemWriter<NotificationDto> notificationDtoItemWriter
    ) {
        return stepBuilderFactory.get("aptNotificationStep")
                ...
                .writer(notificationDtoItemWriter)
                .build();
    }
    ...
    @StepScope
    @Bean
    public ItemWriter<NotificationDto> notificationDtoItemWriter() {
        return items -> items.forEach(item -> log.info(item.toMessage()));
    }
}
